...
def resolve_agent_arn() -> str:
    ...
    import yaml
    config_file = os.path.join(SAMPLE_DIR, ".bedrock_agentcore.yaml")
    if not os.path.exists(config_file):
        raise FileNotFoundError("No .bedrock_agentcore.yaml found. Run 'agentcore configure' and 'agentcore launch' first.")
    with open(config_file) as f:
        cfg = yaml.safe_load(f)
    default_agent = cfg.get("default_agent")
    arn = cfg.get("agents", {}).get(default_agent, {}).get("bedrock_agentcore", {}).get("agent_arn")
    if arn:
        return arn
...
def resolve_gateway_url() -> str:
    """Read the gateway URL from AWS."""
    try:
        with open(os.path.join(SAMPLE_DIR, "cognito_config.json")) as f:
            cfg = json.load(f)
        ctrl = boto3.client("bedrock-agentcore-control", region_name=cfg["region"])
        for gw in ctrl.list_gateways().get("items", []):
            if gw.get("name") == "MyGateway":
                return ctrl.get_gateway(gatewayIdentifier=gw["gatewayId"]).get("gatewayUrl", "N/A")
    except Exception:
        pass
    return "N/A"
